...
func (k *Keeper) RejectMemory(ctx context.Context, orgID string, contentHash []byte, rejecter string) error {
	isLeader, err := k.orgKeeper.IsLeader(ctx, orgID, rejecter)
	if err != nil {
		return err
	}
	if !isLeader {
...
func (k *Keeper) PurgeExpired(ctx context.Context, orgID string, currentEpoch uint64) (uint64, error) {
	params, err := k.GetParams(ctx)
	if err != nil {
		return 0, err
	}

	cutoffEpoch := currentEpoch - params.PendingRetentionEpochs
	if params.PendingRetentionEpochs == 0 || currentEpoch <= params.PendingRetentionEpochs {
		cutoffEpoch = 0
	}
	...
	for ; iter.Valid(); iter.Next() {
		...
		if stored.Epoch < cutoffEpoch {
			purgeKeys = append(purgeKeys, iter.Key())
		}
	}
	...
	k.logger.Info("expired commitments purged",
		"org_id", orgID,
