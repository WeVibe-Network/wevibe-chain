...
func TestRejectMemory(t *testing.T) {
	...
	err := k.RejectMemory(ctx, "test-org", contentHash, "leader-pubkey")
	if err != nil {
	...
	if err != types.ErrMemoryNotFound {
		t.Fatalf("expected no approved memory, got: %v", err)
	}
}

func TestRejectMemory_NotLeader(t *testing.T) {
	k, _, _ := makeTestKeeper(t)
	ctx := context.Background()
	...
	err := k.RejectMemory(ctx, "test-org", contentHash, "not-leader")
	if err != types.ErrUnauthorized {
		t.Fatalf("expected ErrUnauthorized, got: %v", err)
	}
}
...
func TestPurgeExpired(t *testing.T) {
	...
	count, err := k.PurgeExpired(ctx, "test-org", 10)
	if err != nil {
	...
	if len(pending) != 1 {
		t.Fatalf("expected 1 pending remaining, got: %d", len(pending))
	}
}
